<template>
	<view v-if="pay">
		<view style="text-align: center; font-weight: bold; margin: 100rpx 0;">支付成功{{info.price}}元</view>
		...
		<navigator hover-class="none" :url="'/pages/hp/product_detail?id='+pid+'&to_uid='+to_uid+'&name='+name" open-type="redirect" style="text-align: center;"><button size="mini">返回上一页</button></navigator>
	</view>
	<view v-else>
...
<script>
	...
	export default {
		...
		methods: {
			pays(){
				uni.request({
					url:app.globalData.url+'hp_product_detail/pay',
					method:'POST',
			...
			getInfo(){
				uni.request({
					url:app.globalData.url+'hp_product_detail/detail',
					data:{id:that.id},
					success(res) {
